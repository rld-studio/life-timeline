...
export function ImageManager({ eventId, eventColor, coverImage, artifacts, onCoverChange, onArtifactsChange }: Props) {
  ...
  async function handleRemoveCover() {
    ...
    setError(null)
    try { await deleteImage(coverImage); onCoverChange(undefined) }
    catch (e: any) { setError(e.message) }
  }

  async function handleGalleryFiles(files: FileList) {
    setError(null); setGalleryUploading(true)
    try {
      const newImgs: Artifact[] = []
      for (const file of Array.from(files)) {
        const { url } = await uploadImage(file, eventId, 'gallery')
        newImgs.push({ type: 'image', url, caption: '' })
      }
      onArtifactsChange([...otherArtifacts, ...galleryImages, ...newImgs])
    } catch (e: any) { setError(e.message) }
  ...
  async function handleRemoveGalleryImage(url: string) {
    ...
    try {
      ...
      onArtifactsChange(artifacts.filter(a => !(a.type === 'image' && a.url === url)))
    } catch (e: any) { setError(e.message) }
  }

  function handleCaptionChange(url: string, caption: string) {
    onArtifactsChange(artifacts.map(a => a.type === 'image' && a.url === url ? { ...a, caption } : a))
  }
  ...
  return (
    <div className="img-manager">
        ...
        {coverImage ? (
          <div className="img-cover-preview">
            ...
            <div className="img-cover-actions">
              ...
              <button className="img-btn img-btn--danger" onClick={handleRemoveCover} disabled={coverUploading}>✕ Remove</button>
            </div>
          </div>
        ) : (
          <div className="img-drop-zone img-drop-zone--cover"
            style={{ '--c': eventColor } as React.CSSProperties}
            onDragOver={e => e.preventDefault()} onDrop={e => onDrop(e, 'cover')}
            onClick={() => coverInputRef.current?.click()}>
            {coverUploading ? <span className="img-drop-label">Uploading…</span> : <>
              <span className="img-drop-icon">🖼</span>
              <span className="img-drop-label">Drop or click to add cover photo</span>
              <span className="img-drop-hint">JPG, PNG, WebP</span>
            </>}
          </div>
        )}
        <input ref={coverInputRef} type="file" accept={ACCEPT} style={{ display: 'none' }}
      ...
      <section className="img-section">
            ...
            {galleryImages.map((img, i) => (
              <div key={img.url ?? i} className="img-gallery-item">
                <div className="img-gallery-thumb-wrap">
                  ...
                  <button className="img-gallery-remove" onClick={() => img.url && handleRemoveGalleryImage(img.url)}>✕</button>
                </div>
                <input type="text" className="img-caption-input" placeholder="Caption (optional)"
                  value={img.caption ?? ''}
        ...
        <div className="img-drop-zone img-drop-zone--gallery"
          onDragOver={e => e.preventDefault()} onDrop={e => onDrop(e, 'gallery')}
          onClick={() => galleryInputRef.current?.click()}>
          {galleryUploading ? <span className="img-drop-label">Uploading…</span> : <>
            <span className="img-drop-icon">＋</span>
            <span className="img-drop-label">Drop or click to add images</span>
            <span className="img-drop-hint">Multiple files OK</span>
          </>}
        </div>
        <input ref={galleryInputRef} type="file" accept={ACCEPT} multiple style={{ display: 'none' }}
