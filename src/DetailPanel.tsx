      ...
      <div className="detail-scroll">
        ...
        {showAddForm ? (
          ...
          <div className="detail-add-row">
            ...
            <button className="detail-add-btn" onClick={() => setShowAddForm(true)}>
              + Add Event
            </button>
          </div>
        )}
    ...
    <article className={`evt-card${isFirst ? ' evt-card--first' : ''}`}>
      {evt.cover_image ? (
        <div className="evt-cover" onClick={() => setLightboxIdx(0)}>
          ...
          <button className="evt-expand" title="Expand" onClick={e => { e.stopPropagation(); setLightboxIdx(0) }}>⤢</button>
        </div>
      ) : (
